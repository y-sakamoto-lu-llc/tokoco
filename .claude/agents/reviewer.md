...
`type: impl` の PR を**承認**する際、画面から挙動確認できる機能が含まれる場合は、レビュー結果の末尾に以下のセクションを追加してユーザーに提示する。

```
## 手動検証手順

ローカルで以下の手順を実施して動作を確認してください。

### 前提
- `pnpm dev` でローカルサーバーが起動していること
- （必要に応じて）Supabase ローカル環境が起動していること（`supabase start`）
...
**提示する条件（両方を満たす場合）:**
- `type: impl` の PR である
- ページ遷移・フォーム送信・ナビゲーション・エラー表示など、ブラウザ上で確認できる挙動が実装されている

**提示しない場合:**
- `type: design`（設計書のみの変更）
- `type: infra` / `type: chore`（コード変更が画面に現れない）
- テスト・設定ファイルのみの変更
