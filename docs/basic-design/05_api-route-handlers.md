...
Supabase Auth で「メール確認を必須」に設定している場合、確認メールのリンクをクリックしていないユーザーがログインを試みると `signInWithPassword` がエラーを返す。その場合は以下の形式で 401 を返す:
...
Supabase のエラーコードが `email_not_confirmed` の場合にこのメッセージを返す。それ以外の認証エラーは汎用の `{ "error": "メールアドレスまたはパスワードが正しくありません" }` を返す。
...
3. `share_token` を `crypto.randomUUID()` で生成。UUID v4 はランダム部分が 122bit（バージョン・バリアントの固定ビットを除く）であり、SEC-07 の「128bit 以上のランダム性」の要件に対しては 122bit となる。推測困難性の観点から実用上問題ないと判断し採用する（Edge Runtime 対応の Web Crypto API を使用）
4. `events` に INSERT
...
export async function validateShareToken(
  ...
  // リクエストスコープで生成すること（モジュールスコープでの変数保持禁止）
  // セクション4の方針: service_role クライアントはリクエストごとに生成し、グローバルに保持しない
...
**注意（Edge Runtime の制約）:** Cloudflare Pages の Edge Runtime はリクエストごとに独立した isolate で実行されるため、**IN-MEMORY Map によるカウントはリクエストをまたいで保持されない**。モジュールスコープの `Map` を使ったレートリミットは Node.js サーバー環境でのみ有効であり、Cloudflare 環境では機能しない。v1.0 では Supabase Auth の組み込みレート制限を主要な対策として依存し、カスタムレートリミットの実装は見送る。本格的なレートリミットは v2.0 で **Cloudflare Workers KV** または **Upstash Redis** を使用して実装する。
