...
- 背景: `bg-neutral-100` / テキスト: `text-neutral-700`
  - `bg-primary-100`（`#e3f1c7`）は `16_brand-design-system.md` セクション2.2 の規定により装飾専用（テキスト配置禁止）のため使用不可。代替として `neutral-100`（`#f4f4f5`）+ `neutral-700`（`#3f3f46`）を使用する（コントラスト比: 約10.7:1、WCAG AA 合格）。
- 「×」ボタンは `aria-label="タグ名 を削除"` を付与する（A11Y-02）。

### 6.4 SHOP-13: 使用0件タグ自動削除後のサジェスト更新（UI方針）

タグを店舗から取り外した際に、そのタグの使用件数が0になった場合、サーバー側で自動削除が行われる（SHOP-13）。
UI側は以下の方針で対応する:

- `TagBadge` の「×」ボタン押下後、サーバーへの削除リクエストのレスポンスを受け取ってから、`TagInput` が保持するサジェストリストを再取得する（例: `GET /api/tags` を呼び出し）。
- 再取得したサジェストリストで `suggestions` を更新することで、自動削除されたタグが次回以降のドロップダウンに表示されなくなる。
- 再取得中はサジェストドロップダウンへの操作を妨げない（バックグラウンドで更新し、ユーザー操作を継続可能にする）。
...
- **A11Y-01 対応（WCAG 1.4.1 色の使用）:** 最多得票の判別を色だけに依存させない。最多ハイライト時はアイコン（例: `★`）またはテキストラベル（例: 「最多」）を色と併用して表示すること。アイコンのみの場合は `aria-label="最多得票"` を付与する。
...
- `primary-100`〜`primary-300` は装飾（背景塗り・ボーダー）のみに使用し、その上にテキストを配置しない。タグバッジには `neutral-100`（`#f4f4f5`）+ `neutral-700`（`#3f3f46`）を使用する（セクション6.3参照）。
