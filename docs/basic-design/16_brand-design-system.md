...
| `#ffffff`（白） | `#F59E0B`（secondary-500） | **約2.8:1** | **不合格** | 白文字不可（代わりに黒文字 `#1C1917` を使用）|
| `#1C1917`（stone-900） | `#F59E0B`（secondary-500） | **約9.0:1** | 合格 | セカンダリカラー上のテキスト |
...
- `secondary-500`（暫定: `#F59E0B`）を背景にする場合、**白文字（`#ffffff`）はコントラスト比約2.8:1でA11Y-03を満たさないため使用禁止**。代わりに黒文字 `#1C1917`（stone-900）を使用する。
...
| コード・等幅 | `JetBrains Mono` | Google Fonts 経由（v1.0 では使用箇所なし・`layout.tsx` には未追加。コード表示機能の追加時に導入する） |
...
> **注記:** Disabled 状態は WCAG 1.4.3（コントラスト比）の適用外（[WCAG 2.1 例外規定](https://www.w3.org/TR/WCAG21/#contrast-minimum)）。上記の色指定はデザインの一貫性のための推奨値であり、コントラスト比の達成義務はない。
...
const config: Config = {
  content: [
    // "./src/pages/**/*.{js,ts,jsx,tsx,mdx}",  // App Router 構成では不要
...
// JetBrains Mono は v1.0 では使用箇所なし（コード表示機能が追加される際に導入する）
...
@layer base {
  :root {
    ...
    --secondary: 38 92% 50%;
    --secondary-foreground: 24 10% 10%;  /* stone-900: A11Yコントラスト比確保 */
