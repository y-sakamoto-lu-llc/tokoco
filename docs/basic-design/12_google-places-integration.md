...
|---|---|---|
| `GET /api/shops/places/search` | Text Search（New） | キーワード検索・最大5件の候補を返す |
...
export function convertPriceLevel(priceLevel: string | undefined): string | null {
...
export async function extractPlaceIdFromUrl(url: string): Promise<{ placeId: string } | { keyword: string } | null> {
  ...
  // パターン3: Short URL の展開（Edge Runtime で fetch を使用）
  if (url.includes('goo.gl') || url.includes('maps.app.goo.gl')) {
    const response = await fetch(url, { method: 'HEAD', redirect: 'follow' })
    return extractPlaceIdFromUrl(response.url)
  }
...
3. Route Handler → Google Text Search API（最大5件）
